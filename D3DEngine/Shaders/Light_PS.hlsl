...
SamplerState spBRDF_Sampler : register(s3);
...
struct LightData
{
	...
	float4 components; // [x,y,z,1] - position, [x,y,z,0] - direction
...
float shadowMapVisibility(float3 P, float3 L, float3 N)
{
	...
	if (PL.z > 1.0 || PL.z < 0.0)
		return 0.0;
	float closestDepth = shadowMap.Sample(defaultSampler, PL.xy).r;
...
float3 Uncharted2ToneMapping(float3 color)
{
	...
	color /= white;
	color = pow(color, float3(1.f / gamma));
	return color;
}
...
float4 main(PS_INPUT input) : SV_Target
{
	...
	// Normal
	float3 N = GBuffer_normal.Sample(pointSampler, input.texcoord);
	...
	float3 Li = normalize(light.components.xyz - worldPosition.xyz * light.components.w);
	...
	float vis = shadowMapVisibility(worldPosition.xyz, Li, N);
	float3 result = (1.0 - vis) * directLighting + ambientLighting;

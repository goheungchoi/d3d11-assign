...
core::UUID::UUID(const uint8_t* data) { memcpy(byte, data, kNumBytes); }

core::UUID::UUID(const std::string& str) {
...
bool core::UUID::IsNil() { return uuids::uuid(byte).is_nil(); }

std::string core::UUID::ToString(bool separated /* = false */) {
  std::span sp(byte);
...
bool core::UUID::operator==(const core::UUID& other) const { return false; }

bool core::UUID::IsValidUUIDString(const std::string& uuid) {
  return uuids::uuid::is_valid_uuid(uuid);
}

core::UUID core::GenerateRandomUUID() {
  static UUIDRandomGenerator random_generator{};
  const uuids::uuid id = random_generator.GenerateUUID();
  return core::UUID(reinterpret_cast<const uint8_t*>(id.as_bytes().data()));
}

core::UUID core::GenerateUUIDFromName(const std::string& name) {
  ...
  return core::UUID(reinterpret_cast<const unsigned char*>(id.as_bytes().data()));
}

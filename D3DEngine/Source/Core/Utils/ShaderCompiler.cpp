...
static void Helper(std::vector<std::vector<uint8_t>>& outCombs,
                   ...
                   const std::vector<uint8_t>& selections, size_t currIndex,
                   std::vector<uint8_t>& currComb) {
...
static void GetCombinations(std::vector<std::vector<uint8_t>>& outCombs,
  ...
  if (numVars == 0 || selections.empty()) return;
...
bool DX::GenerateShaderVariantsFile(const char* shaderDir) { 

...
Mesh::Mesh(const struct aiMesh* mesh) {
  assert(mesh->HasPositions());
  assert(mesh->HasNormals());

  _vertices.reserve(mesh->mNumVertices);
  for (size_t i = 0; i < _vertices.capacity(); ++i) {
    Vertex vertex;
  ...
  _faces.reserve(mesh->mNumFaces);
  for (size_t i = 0; i < _faces.capacity(); ++i) {
    assert(mesh->mFaces[i].mNumIndices == 3);
    _faces.push_back({mesh->mFaces[i].mIndices[0], mesh->mFaces[i].mIndices[1],

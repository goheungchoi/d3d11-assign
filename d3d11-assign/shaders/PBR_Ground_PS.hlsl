  ...
  // float roughness = roughnessTexture.Sample(defaultSampler, input.texcoord).r;
	...
	if (usePCF)
	{
		...
		for (int i = 0; i < 9; ++i)
		{
			...
			shadowFactor += shadowTexture.SampleCmpLevelZero(shadowSampler, sampleUV, currShadowDepth - 0.000001);
		}
	...
	{
		float shadowDepth = shadowTexture.Sample(defaultSampler, uv).r;
		if (currShadowDepth > shadowDepth + 0.001)
			shadowFactor = 0.f;
	}
	
	// Get the result color
	float3 result = shadowFactor * directLighting + ambientLighting * 0.2f;
	
	return float4(pow(result, 1 / gamma), 1.f);

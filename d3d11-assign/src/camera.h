...
public:
	...
	Camera(UINT width, UINT height, float vfov = PI_F / 2.f, float nearPlane = 0.01f, float farPlane = 10000.f)
  ...
  float GetFarPlane() const { return farPlane; }
	...
	void MultiplyMoveSpeed(float m) { 
		this->moveSpeed *= m;
	}
  ...
  void MultiplyRotationSpeed(float m) { 
		this->rotationSpeed *= m;
	}

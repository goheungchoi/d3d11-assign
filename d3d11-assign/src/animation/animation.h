 ...
 public:
  ...
  Animation(const std::string& animationPath, Model* model) {
    ...
    const aiScene* scene = importer.ReadFile(
        animationPath, aiProcess_Triangulate | aiProcess_ConvertToLeftHanded);
    ...
    _globalInverseTransform =
        XMMatrixTranspose(XMMATRIX(&scene->mRootNode->mTransformation.a1));
 ...
 private:
  ...
  void ReadMissingBones(const aiAnimation* animation, Model* model) {
    ...
    for (int i = 0; i < numChannels; ++i) {
      ...
      _bones.push_back(Bone(boneName, modelBoneInfoMap[boneName].id, channel));
    }

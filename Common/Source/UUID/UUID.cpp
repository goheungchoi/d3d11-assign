...
ns::UUID::UUID(const uint8_t* data) { memcpy(byte, data, kNumBytes); }
...
bool ns::UUID::IsNil() { return uuids::uuid(byte).is_nil(); }
...
ns::UUID ns::GenerateUUIDFromName(const std::string& name) {
  ...
  return ns::UUID(
      reinterpret_cast<const unsigned char*>(id.as_bytes().data()));
}
